...
export class DataGenerator {
   ...
   */
  generate(schema: Schema, path: string = 'root'): any {
  ...
  private generateString(schema: Schema): string {
    ...
    switch (format) {
      ...
      default:
        ...
        if ((schema as any)['x-faker']) {
          return this.generateWithFaker((schema as any)['x-faker']);
        }
        // Check for pattern
        if (schema.pattern) {
   ...
   * Generate an array
   */
  private generateArray(schema: Schema, path: string): any[] {
   ...
   * Generate an object
   */
  private generateObject(schema: Schema, path: string): Record<string, any> {
  ...
  private generateFromPattern(pattern: string): string {
    ...
    if (pattern.includes('^\\d+$')) return faker.string.numeric();
    if (pattern.includes('^\\w+$')) return faker.string.alphanumeric();
    if (pattern.includes('@')) return faker.internet.email();
   ...
   */
  private mergeSchemas(schemas: Schema[]): any {
   ...
   */
  private generatePlaceholder(ref: string): any {
